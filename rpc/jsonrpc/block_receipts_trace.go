...
func (api *APIEthTraceImpl) GetBlockReceiptsTrace(ctx context.Context, numberOrHash rpc.BlockNumberOrHash) (map[string]interface{}, error) {
	...
	signers, signer_err := api.borImpl.GetSigners(&blockNumber)
	if signer_err == nil {
		block_trxs_enriched["signers"] = signers
	}
	...
	for _, receipt := range receipts {
		...
		full_result := ethutils.MarshalReceipt(receipt, txn, chainConfig, block.HeaderNoCopy(), txn.Hash(), true, true)
	...
	for i := 0; i < trx_with_senders_len; i++ {
		trx := block_trxs_enriched["transactions"].([]interface{})[i].(*ethapi.RPCTransaction)
		...
		if enable_testing && trx.Type != types.AccountAbstractionTxType {
