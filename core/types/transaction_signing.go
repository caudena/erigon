...
func (sg Signer) Sender(tx Transaction) (libcommon.Address, []byte, error) {
	return sg.SenderWithContext(secp256k1.DefaultContext, tx)
}

// SenderWithContext returns the sender address of the transaction.
func (sg Signer) SenderWithContext(context *secp256k1.Context, txn Transaction) (libcommon.Address, []byte, error) {
	var V uint256.Int
	var R, S *uint256.Int
	...
	case *LegacyTx:
		if !t.Protected() {
			if !sg.unprotected {
				return libcommon.Address{}, nil, fmt.Errorf("unprotected txn is not supported by signer %s", sg)
			}
			signChainID = nil
			V.Set(&t.V)
		} else {
			if !sg.protected {
				return libcommon.Address{}, nil, fmt.Errorf("protected txn is not supported by signer %s", sg)
			}
			if !DeriveChainId(&t.V).Eq(&sg.chainID) {
				return libcommon.Address{}, nil, ErrInvalidChainId
			}
	...
	case *AccessListTx:
		if !sg.accessList {
			return libcommon.Address{}, nil, fmt.Errorf("accessList txn is not supported by signer %s", sg)
		}
		if t.ChainID == nil {
	...
	case *DynamicFeeTransaction:
		if !sg.dynamicFee {
			return libcommon.Address{}, nil, fmt.Errorf("dynamicFee txn is not supported by signer %s", sg)
		}
		if t.ChainID == nil {
			if !sg.chainID.IsZero() {
				return libcommon.Address{}, nil, ErrInvalidChainId
	...
	case *BlobTx:
		if !sg.blob {
			return libcommon.Address{}, nil, fmt.Errorf("blob txn is not supported by signer %s", sg)
		}
		if t.ChainID == nil {
			if !sg.chainID.IsZero() {
				return libcommon.Address{}, nil, ErrInvalidChainId
	...
	case *SetCodeTransaction:
		if !sg.setCode {
			return libcommon.Address{}, nil, fmt.Errorf("setCode tx is not supported by signer %s", sg)
		}
		if t.ChainID == nil {
			if !sg.chainID.IsZero() {
				return libcommon.Address{}, nil, ErrInvalidChainId
		...
		R, S = &t.R, &t.S
	default:
		return libcommon.Address{}, nil, ErrTxTypeNotSupported
	}
...
func recoverPlain(context *secp256k1.Context, sighash libcommon.Hash, R, S, Vb *uint256.Int, homestead bool) (libcommon.Address, []byte, error) {
	if Vb.BitLen() > 8 {
		return libcommon.Address{}, nil, ErrInvalidSig
	}
	V := byte(Vb.Uint64() - 27)
	...
	if len(pub) == 0 || pub[0] != 4 {
		return libcommon.Address{}, nil, errors.New("invalid public key")
	}
